...
for i in range(sizeTodos):
    ...
    r = requests.get(url=URL + "/todos?title=" + todosTitle.replace(" ","%20"))
    print(r.json())

for i in range(sizeProjects):
    projectsTitle = projects.json().get("projects")[i].get("title")
    r = requests.get(url=URL + "/projects?title=" + projectsTitle.replace(" ","%20"))
    print(r.json())

for i in range(sizeCategories):
    categoriesTitle = categories.json().get("categories")[i].get("title")
    r = requests.get(url=URL + "/categories?title=" + categoriesTitle.replace(" ","%20"))
    print(r.json())